...
var slot = [];
export default function Bookcliniccalender({navigation, route}) {
  ...
  var d = new Date();
  useEffect(() => {
    console.log('********* BookClinicCalendar **********');
    ...
    console.log('doc ids ', doctor_details.doc_id);
    const booking = firestore()
      .collection('appointment')
      .doc(doctor_details.doc_id.toString())
      .onSnapshot(documentSnapshot => {
  ...
  function createSlots(day, date) {
    ...
    if (doctor_details.clinics[clinic_details].days.includes(day)) {
      setSlots(
        doctor_details.clinics[clinic_details].timing,
        date,
        doctor_details.clinics[clinic_details].id,
      );
    }
  }
  return (
