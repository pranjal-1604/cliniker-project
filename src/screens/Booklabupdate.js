...
export default function BooklabCalender({navigation, route}) {
  ...
  var d = new Date();
  useEffect(() => {
    console.log(cart, null, 2);
    console.log(
      'span ',
      cart[index].clinic.time.outime - cart[index].clinic.time.intime,
    );
    setSpan(cart[index].clinic.time.outime - cart[index].clinic.time.intime);
      ...
      slot.push(
        {
          ...
          labtest_name: sources.test_name,
          lab_name: sources.lab_name,
          fees: sources.fees,
          reschedule_times: 0,
          review: '',
          collection_option: {
