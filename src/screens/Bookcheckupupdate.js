...
var slot = [];
export default function Bookcheckupupdate({navigation, route}) {
  ...
  useEffect(() => {
    ...
    if (cart?.ordered == true) {
      const booking = firestore()
        ...
        .doc(cart.data[index].doctor.doc_id)
        .onSnapshot(documentSnapshot => {
  ...
  function addtocart() {
    ...
    if (cart?.ordered) {
      cart.data[index] = selectedslotdetails;
      firestore()
        .collection('doctor_cart')
        .doc(uid)
        .set({
          appointments: cart.data,
        })
        .then(() => {
          navigation.navigate('Cart');
        });
    } else {
      cart[index] = selectedslotdetails;
      firestore()
        .collection('doctor_cart')
        .doc(uid)
        .set({
      ...
      slot.push(
        {
          ...
          clinic_id: id,
          doctor: cart?.ordered ? cart.data[index].doctor : cart[index].doctor,
          reschedule_times: cart?.ordered
            ? cart.data[index].reschedule_times + 1
            : cart[index].reschedule_times + 1,
          status: {
            booked: false,
            consultation: false,
            prescription: false,
  ...
  function createSlots(day, date) {
    ...
    console.log('clinicss', cart[index]);
    if (cart?.ordered == true) {
      for (var x = 0; x < cart.data[index].doctor.clinics.length; x++) {
        if (cart.data[index].doctor.clinics[x].days.includes(day)) {
          setSlots(
            cart.data[index].doctor.clinics[x].timing,
            date,
            cart.data[index].doctor.clinics[x].id,
          );
        }
      }
    } else {
      for (var x = 0; x < cart[index].doctor.clinics.length; x++) {
        if (cart[index].doctor.clinics[x].days.includes(day)) {
          setSlots(
            cart[index].doctor.clinics[x].timing,
            date,
            cart[index].doctor.clinics[x].id,
          );
        }
      }
    }
