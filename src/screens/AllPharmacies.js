...
export default function AllPharmacies(props) {
  ...
  const getPharmacies = async () => {
    var pharmacy_list = [];
    const subscriber = firestore().collection('pharmacy');
    const snapshot = await subscriber.get();
    snapshot.docs.map(doc => {
      // console.log(doc.data());
      pharmacy_list.push(doc.data());
    });
    // console.log('Pharmacies:', pharmacy_list);
    setpharmacies(pharmacy_list);
  };
  ...
  dialCall = phoneNumber => {
    ...
    if (Platform.OS === 'android') {
      phoneNumber = 'tel:${+91' + phoneNumber + '}';
    } else {
      phoneNumber = 'telprompt:${+91' + phoneNumber + '}';
    }
                  ...
                  <View style={{paddingHorizontal: 6}}>
                    ...
                    <View
                      ...
                      <Text style={styles.speciality}>
                        Location,{pharmacy.location.city}
                      </Text>
                    </View>
                    <TouchableOpacity
                      onPress={() => dialCall(pharmacy.number)}
                      style={[
                        styles.button,
                        {marginTop: 12, width: 111, flexDirection: 'row'},
                      ]}>
                      <Text style={styles.buttonT}>Make a Call</Text>
                      <Image
